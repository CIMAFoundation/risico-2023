...
pub fn store_day_fn(state: &mut OrieuxStateElement, input: &InputElement) {
...
pub fn update_fn(
    ...
    if (state.min_temp) != NODATAVAL && (state.max_temp != NODATAVAL) {
        ...
        let pet = evapotranspiration_thornthwaite(temp_eff_corr, prop.lat, *time, prop.heat_index);
        state.pet = pet;
        let orieux_wr_new = f32::min(
            R_MAX,
            state.orieux_wr + state.cum_rain - (state.orieux_wr / R_MAX) * state.pet,
        )
        .clamp(0.0, R_MAX);
...
pub fn get_output_fn(state: &OrieuxStateElement) -> OutputElement {
    OutputElement {
