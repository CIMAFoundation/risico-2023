...
pub fn store_day_fn(state: &mut KBDIStateElement, input: &InputElement) {
...
pub fn kbdi_update_mm(
    ...
    history_rain: &[f32], // daily rain of the last days + rain of today [mm] (ordered from oldest to newest)
    ...
    let evapo_transp: f32 =
        (((203.2 - qsi) * (0.968 * f32::exp(0.0875 * max_temp + 1.5552) - 8.3) * dt)
            / (1.0 + 10.88 * f32::exp(-0.001736 * mean_annual_rain)))
            * 10e-3;
    let kbdi_new = qsi + evapo_transp;
    kbdi_new.clamp(0.0, 200.0)
}
...
pub fn get_output_fn(state: &KBDIStateElement) -> OutputElement {
    OutputElement {
        kbdi: state.kbdi,            // [mm]
