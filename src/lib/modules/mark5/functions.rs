...
pub fn ffdi(
    ...
    2.0 * f32::exp(
        -0.45 + 0.987 * f32::ln(drought_factor) - 0.0345 * rh + 0.0338 * temp + 0.0234 * ws_kph,
    )
}
...
pub fn drought_factor(
    ...
    dates: &[DateTime<Utc>], // dates associated with the dalily_rain vector
    ...
    let min_rain_effect = rain_effects
        .iter()
        .cloned()
    ...
    let df = 10.5
        * (1.0 - f32::exp(-(smd + 30.0) / 40.0))
        * ((41.0 * f32::powf(rain_effect_eff, 2.0) + rain_effect_eff)
            / (40.0 * f32::powf(rain_effect_eff, 2.0) + rain_effect_eff + 1.0));
...
pub fn get_output_fn(
    ...
    // calculate the FFDI
    let ffdi = if (state.temp_15 == NODATAVAL)
        || (state.humidity_15 == NODATAVAL)
        || (state.wind_speed_15 == NODATAVAL)
    {
        NODATAVAL
    } else {
