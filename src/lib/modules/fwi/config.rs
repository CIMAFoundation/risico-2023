...
use super::functions::{
    get_output_legacy, get_output_sliding, update_state_legacy, update_state_sliding,
};
use super::models::{FWIPropertiesElement, FWIStateElement};
...
impl FWIModelConfig {
    pub fn new(model_version_str: &str) -> Self {
        let update_state_fn: fn(
            &mut FWIStateElement,
            &FWIPropertiesElement,
            &InputElement,
            &DateTime<Utc>,
        );
        let get_output_fn: fn(
            &mut FWIStateElement,
            &FWIPropertiesElement,
            &DateTime<Utc>,
        ) -> OutputElement;
